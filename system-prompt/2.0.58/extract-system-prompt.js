 ...
 */
const VAR_MAP = {
  // 2.0.57 variable names (updated from 2.0.56)
  'C9': 'Bash',
  'r8': 'Task',
  'd7B': 'TodoWrite',
  'u5': 'Read',
...
function replaceVariables(text) {
  // Simple variable references (2.0.57 variable names)
  text = text.replace(/\$\{D9\}/g, 'Bash');
  text = text.replace(/\$\{s8\}/g, 'Task');
  text = text.replace(/\$\{gGB\.name\}/g, 'TodoWrite');
  text = text.replace(/\$\{gGB\}/g, 'TodoWrite');
  text = text.replace(/\$\{g5\}/g, 'Read');
  text = text.replace(/\$\{R5\}/g, 'Edit');
  text = text.replace(/\$\{bX\}/g, 'Write');
  text = text.replace(/\$\{vX\}/g, 'WebFetch');
  text = text.replace(/\$\{CD\}/g, 'Glob');
  text = text.replace(/\$\{uY\}/g, 'Grep');
  text = text.replace(/\$\{dJ\}/g, 'AskUserQuestion');
  text = text.replace(/\$\{O_\}/g, 'WebSearch');
  text = text.replace(/\$\{yP\}/g, 'SlashCommand');
  text = text.replace(/\$\{Uf1\}/g, 'claude-code-guide');

  // Object.name patterns (2.0.57)
  text = text.replace(/\$\{In\.name\}/g, 'Task');
  text = text.replace(/\$\{FO\.name\}/g, 'Glob');
  text = text.replace(/\$\{d8\.name\}/g, 'Read');
  text = text.replace(/\$\{oX\.name\}/g, 'Write');
  text = text.replace(/\$\{tI\.name\}/g, 'TodoWrite');
  text = text.replace(/\$\{An\.name\}/g, 'Task');

  // Agent type patterns (2.0.57)
  text = text.replace(/\$\{Sq\.agentType\}/g, 'Explore');
